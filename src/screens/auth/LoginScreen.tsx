...
const LoginScreen = () => {
  ...
  // Enhanced useEffect to ensure installationUrl and locale are set from env
  useEffect(() => {
    // Force update URL from env if available, overriding persisted state
    if (process.env.EXPO_PUBLIC_CHATWOOT_BASE_URL && installationUrl !== process.env.EXPO_PUBLIC_CHATWOOT_BASE_URL) {
       dispatch(settingsActions.setInstallationUrl(process.env.EXPO_PUBLIC_CHATWOOT_BASE_URL));
    }
    // Force update Locale to Spanish
    if (activeLocale !== 'es') {
       dispatch(setLocale('es'));
    }
  }, [installationUrl, activeLocale, dispatch]);
    ...
    <SafeAreaView edges={['top']} style={tailwind.style('flex-1 bg-white')}>
      ...
      <View style={tailwind.style('flex-1 bg-white')}>
        <Animated.ScrollView
          ...
          <View style={tailwind.style('mt-12')}>
            <Button
