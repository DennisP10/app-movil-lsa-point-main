...
const ConversationList = () => {
  ...
  const ListFooterComponent = () => {
    ...
    return (
      <Animated.View
        ...
        {isAllConversationsFetched ? null : <ActivityIndicator size="small" color="#F27D27" />}
      </Animated.View>
    );
  };
  ...
  const fetchConversations = useCallback(
    async (filters: FilterState, page: number = 1) => {
      const isCustomFilterActive = filters.assignee_id && filters.assignee_id !== '0';

      if (isCustomFilterActive) {
        const payloadRules: import('@/store/conversation/conversationTypes').FilterRule[] = [];
        payloadRules.push({
          attribute_key: 'assignee_id',
          filter_operator: 'equal_to' as const,
          values: [parseInt(filters.assignee_id)],
          query_operator: 'and' as const,
        });
        ...
        if (filters.inbox_id && filters.inbox_id !== '0') {
          payloadRules.push({
            ...
            filter_operator: 'equal_to' as const,
            values: [parseInt(filters.inbox_id)],
            query_operator: 'and' as const,
          });
        }

        if (payloadRules.length > 0) {
          payloadRules[payloadRules.length - 1].query_operator = undefined;
        }

        dispatch(conversationActions.fetchFilteredConversations({ payload: payloadRules, page }));
      } else {
  ...
  return shouldShowEmptyLoader ? (
    <Animated.View
      ...
      <ActivityIndicator color="#F27D27" />
    </Animated.View>
    ...
    <SafeAreaView edges={['top']} style={tailwind.style('flex-1 bg-white')}>
      ...
      <ConversationListStateProvider>
        ...
        <BottomSheetModal
          ...
          <React.Fragment>
            {currentBottomSheet === 'status' || currentBottomSheet === 'sort_by' || currentBottomSheet === 'assignee_type' ? (
              <BottomSheetWrapper>
              ...
              <React.Fragment>
                ...
                {currentBottomSheet === 'assignee_id' ? <AssigneeCustomFilters /> : null}
              </React.Fragment>
            )}
          </React.Fragment>
        </BottomSheetModal>
