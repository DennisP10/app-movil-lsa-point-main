...
export class ConversationService {
  ...
  static async filterConversations(payloadData: FilterConversationsPayload): Promise<ConversationListResponse> {
    const { payload, page = 1 } = payloadData;

    const response = await apiService.post<ConversationListAPIResponse>(
      'conversations/filter',
      { payload },
      { params: { page } }
    );
    const responseData = (response.data as any).data || response.data;
    const { payload: conversations, meta } = responseData as any;
    const transformedResponse: ConversationListResponse = {
      conversations: conversations.map(transformConversation),
      meta: transformConversationListMeta(meta),
    };
