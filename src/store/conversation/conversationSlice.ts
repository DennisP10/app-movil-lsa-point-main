...
import { transformAgent, transformTeam } from '@/utils/camelCaseKeys';
...
const conversationSlice = createSlice({
  ...
  extraReducers: builder => {
    builder
      ...
      .addCase(conversationActions.fetchFilteredConversations.pending, state => {
        state.error = null;
        state.isLoadingConversations = true;
      })
      .addCase(conversationActions.fetchFilteredConversations.fulfilled, (state, { payload }) => {
        const { conversations, meta } = payload;
        conversationAdapter.upsertMany(state, conversations);
        state.isLoadingConversations = false;
        state.isAllConversationsFetched = conversations.length < 20 || false;
      ...
      .addCase(conversationActions.fetchFilteredConversations.rejected, (state, { error }) => {
        state.isLoadingConversations = false;
      })
      ...
      .addCase(conversationActions.assignConversation.fulfilled, (state, action) => {
        const { conversationId, assigneeId, teamId } = action.meta.arg;
        const conversation = state.entities[conversationId];
        if (conversation && conversation.meta) {
          if (assigneeId !== undefined) {
             if (assigneeId === 0) {
               conversation.meta.assignee = undefined as any;
             } else {
               const rawAgent = (action.payload as any)?.data?.payload || (action.payload as any)?.payload || action.payload;
               if (rawAgent && typeof rawAgent === 'object' && rawAgent.id) {
                 conversation.meta.assignee = transformAgent(rawAgent) as any;
               }
             }
          }
          if (teamId !== undefined) {
             if (teamId === '0' || teamId === null) {
               conversation.meta.team = undefined as any;
             } else {
               const rawTeam = (action.payload as any)?.data?.payload || (action.payload as any)?.payload || action.payload;
               if (rawTeam && typeof rawTeam === 'object' && rawTeam.id) {
                 conversation.meta.team = transformTeam(rawTeam) as any;
               }
             }
          }
        }
      })
      .addCase(conversationActions.assignTeam.fulfilled, (state, action) => {
        const { conversationId, teamId } = action.meta.arg;
        const conversation = state.entities[conversationId];
        if (conversation && conversation.meta) {
          if (teamId === '0' || !teamId) {
            conversation.meta.team = undefined as any;
      ...
      .addCase(conversationActions.addOrUpdateConversationLabels.fulfilled, (state, action) => {
        const { conversationId, labels } = action.meta.arg;
        const conversation = state.entities[conversationId];
        if (conversation) {
