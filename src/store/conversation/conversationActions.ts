...
export const conversationActions = {
  ...
  fetchFilteredConversations: createAsyncThunk<ConversationListResponse, FilterConversationsPayload>(
    'conversations/fetchFilteredConversations',
    async (payload, { rejectWithValue }) => {
      try {
        return await ConversationService.filterConversations(payload);
      } catch (error) {
        const { response } = error as AxiosError<ApiErrorResponse>;
        if (!response) {
          throw error;
